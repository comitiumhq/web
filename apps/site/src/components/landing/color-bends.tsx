...
void main() {
  ...
  q += toward * uMouseInfluence * 0.2;
    ...
    if (uColorCount > 0) {
      ...
      if (uTransparent > 0) {
        col = colorWeight > 0.0 ? clamp(sumCol / colorWeight, 0.0, 1.0) : vec3(0.0);
        a = cover;
      } else {
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const toColorVector = (color: string) => {
      const variableName = /^var\((--[^)]+)\)$/.exec(color.trim())?.[1];
      const resolvedColor = variableName
      ...
      if (!context) return new THREE.Vector3(0, 0, 0);
    ...
    const arr = (colors || []).filter(Boolean).slice(0, MAX_COLORS).map(toColorVector);
