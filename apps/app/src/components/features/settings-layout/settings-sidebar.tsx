...
export function SettingsSidebar({ entries, pathname }: SettingsSidebarProps) {
  ...
  return (
    <aside className="hidden h-full w-68 shrink-0 py-6 lg:block">
      <nav className="flex h-full min-h-0 flex-col gap-1 overflow-y-auto rounded-2xl border border-surface-border bg-card bg-clip-padding p-4 text-card-foreground">
        {entries.map((entry) => {
          if (entry.type === 'item') {
