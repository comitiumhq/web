...
export function useLinkHandler(props: LinkHandlerProps) {
  ...
  const setLink = useCallback(() => {
    if (!url || !editor || !httpsUrlSchema.safeParse(url).success) return;
  ...
  const openLink = useCallback(
    (target = '_blank', features = 'noopener,noreferrer') => {
      ...
      const safeUrl = httpsUrlSchema.safeParse(url);

      if (safeUrl.success) {
        window.open(safeUrl.data, target, features);
      }
    },
    [url],
  );
  ...
  return {
    url: url || '',
    isUrlValid: !url || httpsUrlSchema.safeParse(url).success,
    setUrl,
    setLink,
    removeLink,
    openLink,
