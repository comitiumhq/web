...
type RefCompatibleElement = React.ReactElement & {
  props: {
...
function getElementRef(child: React.ReactElement): React.Ref<HTMLElement> | undefined {
  const refCompatibleChild = child as RefCompatibleElement;

  return Number.parseInt(version, 10) >= 19 ? refCompatibleChild.props.ref : refCompatibleChild.ref;
}
...
export const TooltipTrigger = forwardRef<HTMLElement, TooltipTriggerProps>(function TooltipTrigger(
  ...
  const childrenRef = isValidElement(children) ? getElementRef(children) : undefined;
