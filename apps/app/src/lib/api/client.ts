...
export {
  ApiError,
  getApiErrorStatus as getErrorStatus,
  hasApiErrorCode,
  hasApiErrorStatus,
  isApiError,
  isTransientApiError,
} from '@comitium/schemas/api-errors';
...
async function createRequestHeaders(initialHeaders?: HeadersInit): Promise<Headers> {
...
async function request<T>(
  ...
  const response = await fetch(`${API_URL}${url}`, {
    method,
    headers: await createRequestHeaders({ 'Content-Type': 'application/json' }),
    credentials: 'include',
...
export const api = {
  ...
  post: <T>(url: string, body?: unknown, schema?: ResponseSchema<T> | null) => request<T>(url, 'POST', body, schema),
