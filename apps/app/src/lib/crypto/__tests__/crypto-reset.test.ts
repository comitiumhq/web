...
describe('performCryptoReset', () => {
  ...
  it('keeps the initiating tab on its requested path after logout', async () => {
    ...
    const clearClientState = vi.fn();
    const unsubscribe = subscribeToCryptoReset({ onResetStart: vi.fn(), onResetComplete: vi.fn() });
    ...
    expect(replace).toHaveBeenCalledExactlyOnceWith('/invite?token=invite-token');
...
describe('subscribeToCryptoReset', () => {
  ...
  it('waits for local teardown before reloading on reset-complete', async () => {
    ...
    channel.onmessage?.({ data: { type: 'reset-start', sourceId: 'other-tab' } } as MessageEvent<CryptoResetMessage>);
    channel.onmessage?.({
