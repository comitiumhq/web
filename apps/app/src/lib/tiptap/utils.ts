...
export const isMarkInSchema = (markName: string, editor: Editor | null): boolean => {
  ...
  return isDefined(editor.schema.spec.marks.get(markName));
};
...
export const isNodeInSchema = (nodeName: string, editor: Editor | null): boolean => {
  if (!editor?.schema) return false;
  return isDefined(editor.schema.spec.nodes.get(nodeName));
};
 ...
 */
export function isValidPosition(pos?: number | null): pos is number {
...
export function findNodePosition(props: {
  ...
  const hasValidNode = isDefined(node);
  const validNodePos = isValidPosition(nodePos) ? nodePos : null;

  if (!hasValidNode && validNodePos === null) {
    return null;
  }
...
export const handleImageUpload = async (
  ...
  // Simulate upload progress. Replace this when real file persistence is wired.
...
const ATTR_WHITESPACE =
  // biome-ignore lint/suspicious/noControlCharactersInRegex: intentional — strips whitespace + control chars from URI attrs (DOMPurify-style).
  /[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g;
 ...
 *               Pass `null` or `undefined` to remove the attribute.
...
export function updateNodesAttr<A extends string = string, V = unknown>(
  ...
  for (const { pos } of targets) {
    ...
    const resolvedNext = typeof next === 'function' ? (next as (p?: V) => V | undefined)(prevValue) : next;

    if (prevValue === resolvedNext) continue;

    const nextAttrs: Record<string, unknown> = { ...currentNode.attrs };
    if (!isDefined(resolvedNext)) {
